...
export const SecurityModal: React.FC<SecurityModalProps> = ({ onClose, onVerify, title, hint }) => {
    ...
    return (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
            <div className={`w-full max-w-sm bg-[#161621] border ${error ? 'border-rose-500 shadow-[0_0_20px_rgba(244,63,94,0.3)]' : 'border-indigo-500/30'} rounded-3xl overflow-hidden shadow-2xl transition-all`}>
                {/* Header */}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="••••••••"
                            className="w-full bg-[#0b0b0f] border border-[#1e1e2d] rounded-2xl py-4 pl-12 pr-4 text-center text-xl font-mono tracking-[0.5em] focus:border-indigo-500 outline-none transition-all placeholder:text-gray-800 placeholder:tracking-normal"
                        />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button
                            type="button"
                            onClick={onClose}
                            className="py-3 px-4 rounded-xl bg-white/5 hover:bg-white/10 text-gray-400 font-bold text-xs uppercase tracking-widest transition-all"
                        >
                            取消
                        </button>
                        <button
